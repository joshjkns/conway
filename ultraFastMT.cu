...
__global__ void multistepKernel(uint64_t* globalData, int height, int width, int iteration) {
    ...
    __shared__ uint64_t warpStorage[1920];

    // 6 columns of 320 rows
    int warpStorageWidth = 6;
    int warpStorageHeight = 320;


    // warpId, 0-15 within the block, arranged in column major order (threadIdx.y >> 5 gives warp row as dividing by 2^5 = 32 each thread handles 2 rows, threadIdx.x * 4 gives warp column)
    int warpId = (ty >> 5) + (tx * 4);
    // LaneId, 0-31 within the warp (31 is 0x1F, which is 2^5 - 1 (binary: 11111))
    int laneId = ty & 31;

    // Global grid X position
    int globalX = (blockIdx.x * blockDim.x) + tx;
    // Global grid X nighbours (wrap-around since width is power of 2)
    int globalXleft  = (globalX - 1) & (width - 1);
    int globalXright = (globalX + 1) & (width - 1);

    // Determine which warp this thread belongs to within the block, but it's a fraction, so 1/2 is the start of the third row of warps, 1/4 is the start of the second row of warps, etc.
    float yQuarter = (ty >> 5) * 0.25f;
    // Where the warp starts and ends in Y within the global grid
    int centralWarpStartY = (blockIdx.y * 256) + (256 * yQuarter);
    int centralWarpEndY = centralWarpStartY + 64;

    // Where the halo rows start and end in Y within the global grid
    ...
    int leftCol = (warpStorageHeight * (warpId / 4));
    int middleCol = (warpStorageHeight * ((warpId / 4) + 1));
    int rightCol = (warpStorageHeight * ((warpId / 4) + 2));


    if (warpId < 4){
        for (int i = 0; i < 2; i++){
            int localY = centralWarpStartY + (laneId * 2) + i;
            ...
            warpStorage[leftCol + warpDepth + 32 + (laneId * 2) + i] = left;
            warpStorage[middleCol + warpDepth + 32 + (laneId * 2) + i] = middle;
        }
        if (warpId == 0){
            int localY = haloStartY + laneId;
            int globalY = (localY) & (height - 1);
            uint64_t left = globalData[(globalY * width) + globalXleft];
            uint64_t middle = globalData[(globalY * width) + globalX];
            warpStorage[leftCol + laneId] = left;
            warpStorage[middleCol + laneId] = middle;
        }
        if (warpId == 3){
            int localY = centralWarpEndY + laneId;
            int globalY = (localY) & (height - 1);
            uint64_t left = globalData[(globalY * width) + globalXleft];
            uint64_t middle = globalData[(globalY * width) + globalX];
            warpStorage[leftCol + 256 + 32 + laneId] = left;
            warpStorage[middleCol + 256 + 32 + laneId] = middle;
        }
    }
    else if (warpId < 8){
        for (int i = 0; i < 2; i++){
        ...
        for (int i = 0; i < 2; i++){
            ...
            warpStorage[middleCol + warpDepth + 32 + (laneId * 2) + i] = middle;
            warpStorage[rightCol + warpDepth + 32 + (laneId * 2) + i] = right;
        }
        if (warpId == 12){
            int localY = haloStartY + laneId;
            int globalY = (localY) & (height - 1);
            ...
            uint64_t right = globalData[(globalY * width) + globalXright];
            warpStorage[middleCol + laneId] = middle;
            warpStorage[rightCol + laneId] = right;
        }
        if (warpId == 15){
            int localY = centralWarpEndY + laneId;
            int globalY = (localY) & (height - 1);
    ...
    if (laneId == 0){
        ...
        r00 = (warpStorage[leftCol + warpDepth + (laneId * 4) - 1] << 32) | (warpStorage[middleCol + warpDepth + (laneId * 4) - 1] >> 32);
        r01 = (warpStorage[middleCol + warpDepth + (laneId * 4) - 1] << 32) | (warpStorage[rightCol + warpDepth + (laneId * 4) - 1] >> 32);
    }
    ...
    uint64_t r21 = (warpStorage[middleCol + warpDepth + (laneId * 4) + 1] << 32) | (warpStorage[rightCol + warpDepth + (laneId * 4) + 1] >> 32);
    uint64_t r30 = (warpStorage[leftCol + warpDepth + (laneId * 4) + 2] << 32) | (warpStorage[middleCol + warpDepth + (laneId * 4) + 2] >> 32);
    uint64_t r31 = (warpStorage[middleCol + warpDepth + (laneId * 4) + 2] << 32) | (warpStorage[rightCol + warpDepth + (laneId * 4) + 2] >> 32);
    uint64_t r40 = (warpStorage[leftCol + warpDepth + (laneId * 4) + 3] << 32) | (warpStorage[middleCol + warpDepth + (laneId * 4) + 3] >> 32);
    uint64_t r41 = (warpStorage[middleCol + warpDepth + (laneId * 4) + 3] << 32) | (warpStorage[rightCol + warpDepth + (laneId * 4) + 3] >> 32);
    ...
    for (int i = 0; i < 32; i++){
        ...
        uint32_t recv_r50_lo = __shfl_down_sync(0xffffffff, r10_lo, 1);
        uint32_t recv_r50_hi = __shfl_down_sync(0xffffffff, r10_hi, 1);
        uint32_t recv_r51_lo = __shfl_down_sync(0xffffffff, r11_lo, 1);
        uint32_t recv_r51_hi = __shfl_down_sync(0xffffffff, r11_hi, 1);

        // Pass r40/r41 DOWN (to laneId+1) and receive from below
        uint32_t recv_r00_lo = __shfl_up_sync(0xffffffff, r40_lo, 1);
        uint32_t recv_r00_hi = __shfl_up_sync(0xffffffff, r40_hi, 1);
        uint32_t recv_r01_lo = __shfl_up_sync(0xffffffff, r41_lo, 1);
        uint32_t recv_r01_hi = __shfl_up_sync(0xffffffff, r41_hi, 1);
